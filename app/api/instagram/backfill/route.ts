...
async function getAllMedia(igUserId: string, maxPages: number) {
  ...
  while (nextUrl && pagesFetched < maxPages) {
    const result = await graphFetch(nextUrl)
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const maxPages = Number(searchParams.get('pages') || '20')
    ...
    if (dryRun) {
      return NextResponse.json({
        ...
        sample_shortcodes: rows.slice(0, 10).map((row) => row.shortcode),
      })
    }
    ...
    if (rows.length === 0) {
      return NextResponse.json({
        success: true,
        message: 'Backfill completed, but no reels were returned',
        page_id: pageId,
        ig_user_id: igUserId,
        pages_fetched: mediaResult.pagesFetched,
    ...
    const successfulRows = rows.filter((row) => row.shortcode)
    ...
    for (const row of successfulRows) {
      ...
      if (linkError) {
        console.error(`Fejl ved linking af reel med shortcode ${row.shortcode}:`, linkError)
      }
    }
